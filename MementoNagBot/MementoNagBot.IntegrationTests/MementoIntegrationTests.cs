...
public class MementoIntegrationTests
{
    ...
    {
        private static readonly string MementoAuthToken =
            Environment.GetEnvironmentVariable("MEMENTO_AUTH_TOKEN")
        ...
        private static IMementoClient GetMementoClient()
        {
            ...
            innerClient.DefaultRequestHeaders.TryAddWithoutValidation(
                "Authorization",
                MementoAuthToken
            );
            return new MementoClient(
                innerClient,
                PollySetupExtensions.GetPolicyRegistry(),
                TestConsoleLoggerFactory.CreateLogger<MementoClient>(LogLevel.Information)
            );
        }
        ...
        public class WhenIAttemptToGetActivitiesForAUser
        {
            ...
            public async Task ThenEachActivityHasADateInRange()
            {
                ...
                res.Skip(1)
                    .SkipLast(1)
                    .Select(te => te.ActivityDate)
                    .ShouldAllBe(ad => ad > startDate && ad < endDate);
            }
